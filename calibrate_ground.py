...
class MasterTestVisualizer:
    ...
    def on_key_press(self, key):
        ...
        if key == keyboard.Key.space:
            # 1. 获取小地图距离
            # 假设 minimap 模块有一个获取当前最近目标距离的方法，如果没有，请根据你的API调整
            # 这里演示假设它返回一个列表，取第一个目标的距离
            targets = self.minimap.get_measured_distance()
            dist = targets.get("Yellow") if targets else 0.0
            
            # 2. 获取高低角比例 (0~1)
            v_ratio = self.elevation.get_vertical_ratio() or 0.0
            
            # 3. 保存数据
            self.captured_data.append({'dist': dist, 'v_ratio': v_ratio})
            print(f"[记录] 距离: {dist:.1f}m, 高度比例: {v_ratio:.3f}")
    ...
    def on_closing(self):
        """程序关闭时排序并输出数据，格式化为可直接复制的 Python 代码"""
        self.is_running = False
        
        if self.captured_data:
            # 排序
            sorted_data = sorted(self.captured_data, key=lambda x: x['dist'])
            
            print("\n" + "#" * 50)
            print("# 捕获的数据 (可直接复制到代码中):")
            print("captured_data_mapping = [")
            for item in sorted_data:
                # 输出成字典列表格式
                print(f"    {{'dist': {item['dist']:.2f}, 'v_ratio': {item['v_ratio']:.4f}}},")
            print("]")
            print("#" * 50 + "\n")
            
            # 如果你需要直接获取坐标数组以便绘图，还可以额外输出这两行：
            dists = [round(x['dist'], 2) for x in sorted_data]
            ratios = [round(x['v_ratio'], 4) for x in sorted_data]
            print(f"# 绘图专用列表：\n# dists = {dists}\n# ratios = {ratios}\n")
...
# 第三人称
# dists = [30.67, 41.39, 58.23, 58.23, 58.23, 110.3, 134.8, 157.77, 229.76, 231.29, 238.95, 240.48, 257.33, 257.33, 340.05]
# ratios = [0.5444, 0.5343, 0.5241, 0.5222, 0.5204, 0.5167, 0.5148, 0.512, 0.5111, 0.5111, 0.5102, 0.5093, 0.5093, 0.5093, 0.5083]

# 第一人称

# dists = [53.61, 58.21, 102.64, 131.74, 157.77, 202.19, 225.16, 286.43, 294.09, 336.98]
# ratios = [0.513, 0.512, 0.5083, 0.5074, 0.5074, 0.5065, 0.5065, 0.5065, 0.5065, 0.5065]